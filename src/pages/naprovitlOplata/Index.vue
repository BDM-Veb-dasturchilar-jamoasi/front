...
export default {
  ...
  methods: {
    ...
    RowClicked(item) {
      this.$router.push({ path: "/naprovitel-oplata-update/" + item.id });
    },

    updateOplataLink(id) {
      return this.$router.push({ path: "naprovitel-oplata-update/" + id });
    },
    creatOplataLink() {
      return this.$router.push({ path: "naprovitel-oplata-create" });
    },
    data() {
      let s = this;
      axios({
        method: "get",
        url: "/naprovitel-uplata/all"
      }).then(res => {
        if (res != undefined) {
    ...
    trash(id) {
      ...
      if (trash) {
        ...
        axios({
          method: "delete",
          url: "/naprovitel-uplata/delete/" + id
        }).then(res => {
          if (res != undefined) {
            self.data();
          }
